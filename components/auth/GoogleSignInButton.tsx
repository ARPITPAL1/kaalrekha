...
import { RefreshCw, AlertCircle, Sparkles, Key, ShieldCheck, X } from "lucide-react";
...
export default function GoogleSignInButton({
  ...
  // Initialize and render Google's official button when script & client ID are ready
  useEffect(() => {
  ...
  const handleFallbackClick = () => {
    ...
    // Try clicking the rendered button inside container
    const innerBtn = gsiContainerRef.current?.querySelector<HTMLElement>(
  ...
  return (
    <div className={`w-full flex flex-col items-center ${className}`}>
      {errorMessage && (
        <div className="w-full mb-3 p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-xs text-museum-mutedRed flex items-center gap-2">
      ...
      <div
        ref={gsiContainerRef}
        className="min-h-[44px] w-full flex items-center justify-center overflow-hidden rounded-xl shadow-xs"
      />

      {/* If Google button isn't rendered or Client ID is missing, show fallback trigger */}
      {!activeClientId && (
        <button
          type="button"
          onClick={handleFallbackClick}
          className="w-full py-3.5 px-4 bg-museum-parchment hover:bg-museum-ivory border border-museum-stone hover:border-museum-terracotta text-museum-charcoal font-sans font-semibold text-xs tracking-wider uppercase rounded-xl transition-all duration-300 shadow-sm hover:shadow flex items-center justify-center gap-3 cursor-pointer"
        >
